import type { Route } from "./+types/gym-update";
...
import { createGymPhoto, updateGym } from "../mutations";
...
export const loader = async ({ request, params }: Route.LoaderArgs) => {
  ...
  const gym = await getGym(client, params.gymId);
    ...
    if (images.length > 0) {
      const photoUrls = await Promise.all(
        images.map(async (file: File, idx: number) => {
          ...
          const filePath = `${params.gymId}/${DateTime.now()}_${idx}`;
      ...
      await Promise.all(
        photoUrls.map(async (url) => {
          if (!url) return;
          await createGymPhoto(client, { gym_id: params.gymId, url });
        }),
      );
    }
...
export default function GymUpdate({
  ...
  const [selectedCity, setSelectedCity] = useState(gym.city);
  ...
  return (
    <div className="mx-auto max-w-screen-lg space-y-8 p-4">
      <h1 className="text-3xl font-bold">체육관 수정</h1>
...
개인 연습 및 3vs3 하기 좋은 체육관입니다.`}
        ...
        <div className="space-y-1">
          ...
          <Select
            ...
            value={selectedCity}
            onValueChange={(value) =>
              setSelectedCity(value as (typeof cityEnum.enumValues)[number])
            }
          >
        ...
        <div className="space-y-1">
          ...
          <Select name="district" defaultValue={gym.district}>
          ...
          <div className="flex gap-8">
            <div className="flex flex-col items-center gap-1">
              <Input
                ...
                defaultChecked={gym.has_water_dispenser}
              />
              <Label
            ...
            <div className="flex flex-col items-center gap-1">
              <Input
                ...
                type="checkbox"
                className="peer hidden"
                defaultChecked={gym.has_heating_cooling}
              />
              <Label
...
실내 전용 운동화를 착용하고 경기를 뛰어주세요.`}
        ...
        <Button className="w-full" disabled={isLoading}>
          {isLoading ? <Loader2Icon className="size-4 animate-spin" /> : "수정"}
        </Button>
