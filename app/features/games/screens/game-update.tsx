import type { Route } from "./+types/game-update";
...
export const action = async ({ request }: Route.ActionArgs) => {
  ...
  try {
    await updateGame(client, { ...formValidation.data });
  } catch (error) {
    if (error instanceof Error)
      return { fieldErrors: undefined, error: error.message };
    return {
      fieldErrors: undefined,
      error: "게스트 모집 수정에 실패하였습니다",
    };
  }

  return redirect(`/games/${formValidation.data.game_id}`);
};

export default function GameCreate({ loaderData }: Route.ComponentProps) {
  const { game: initGame } = loaderData;
  ...
  const [game, setGame] = useState({
    game_id: initGame.game_id,
    gym_id: initGame.gym_id,
    gym_name: initGame.gym.name,
    game_type: initGame.game_type,
    game_gender_type: initGame.game_gender_type,
    description: initGame.description,
    start_date: initGame.start_date,
    start_time: initGame.start_time,
    game_time: initGame.game_time,
    guard: initGame.guard,
    forward: initGame.forward,
    center: initGame.center,
    min_participants: initGame.min_participants,
    max_participants: initGame.max_participants,
    fee: initGame.fee,
    city: initGame.city,
    district: initGame.district,
  });
                            ...
                            <DrawerClose
                              ...
                              onClick={() =>
                                onChangeGym(
                                  gym_id,
                                  name,
                                  city as (typeof cityEnum.enumValues)[number],
                                  district,
                                )
                              }
                            >
